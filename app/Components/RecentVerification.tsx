    ...
    const fetchCompanies = async () => {
      ...
      try {
        ...
        if (Array.isArray(response)) {
          ...
          setTotalCount(response.meta?.pagination?.total || response.data.length);
        } else {
          // Fallback
          setCompanies([]);
          setTotalCount(0);
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="text-gray-400">
            {isLoading ? "Loading..." : `1 - ${Math.min(10, companies.length)} of ${totalCount}`}
          </span>
                  ...
                  <td className="p-3 py-6">
                    <span
                      className={`px-2 py-1 rounded-md text-xs font-medium ${
                        statusColors[company.status] || "bg-gray-600/20 text-gray-400"
                      }`}
                    >
                      {company.status}
                    </span>
